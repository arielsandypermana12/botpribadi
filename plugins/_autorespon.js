    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? `${namabot} tidak aktif` : banned ? 'kamu dibanned' : `Ariel SP di sini, Btw ngapain manggil kangen ya?`,
                footer,
...
╠➥ *Versi Enc* :      *https://github.com/arielsp12/arielsp-aespibotz_vip*
╠➥ *No Enc* : *Rp 40000*
╠➥ *Sc Bot* :        *Masih Beta*
║
╠═〘 PEMBAYARAN 〙 ═
...
╠═〘 ${namabot} 〙 ═`.trim(), footer, 'Dana', '#viadana', 'GoPay', '#viagopay', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`🎗 Bot By Ariel SP || ⏲️ Run Time ${uptime} || 🛰️ Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'}`).catch(_ => _)
